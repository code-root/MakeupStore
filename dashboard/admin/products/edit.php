...
<main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
    ...
    <?php
      if ($_SERVER['REQUEST_METHOD'] == 'GET') { 
        ...
        $sql = "UPDATE products SET name = '$name' , name_en = '$name_en',  categoryID = '$categoryID' ,  price = '$price' , url = '$url'   WHERE id='$id_Products' ";
                
            }else {

        $sql = "UPDATE products SET  name = '$name' ,  name_en = '$name_en' , categoryID = '$categoryID' ,  price = '$price' ,img ='$new_file_name'  , url = '$url'  WHERE id='$id_Products'";
                ...
                <div class="flex justify-center items-center w-12 bg-blue-500">
                    <svg class="h-6 w-6 fill-current text-white" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div class="-mx-3 py-2 px-4">
                            ...
                            <br>
                            <br>
                            <a href="../" class="px-6 py-3 bg-green-600 rounded-md text-white font-medium tracking-wide hover:bg-green-500 ml-3" > الرجوع لكافة المنتجات  </a>
                            ';
               ...
               <a href="../../Upload/product/<?=$row['img']?>" target="_blank" >  <img src="../../../Upload/product/<?=$row['img']?>" height="160" width="160"> </a>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4">
                        ...
                        <div>
                            ...
                            <input class="form-input w-full mt-2 rounded-md focus:border-indigo-600" value="<?=$row['name_en']?>" name="name_en" type="text">
                            <input  value="<?=$row['id']?>"  hidden name="id" type="text">
                        </div>
                        ...
                        <div> 
                                    ...
                                    <?php 
                                    echo categoryGET($row['categoryID']) ;
                                    $categorySQL = "SELECT * FROM category";
                                    $RE = $conn->query($categorySQL);
                                    if ($RE->num_rows > 0) {
                                    while($rowCategory = $RE->fetch_assoc()) { ?>
                        ...
                        <div>
                            ...
                            <input class="form-input w-full mt-2 rounded-md focus:border-indigo-600"  value="<?=$row['url']?>" name="url" type="text">
                        </div>
