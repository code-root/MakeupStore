        ...
        <div class="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
            <div class="align-middle inline-block min-w-full shadow overflow-hidden sm:rounded-lg border-b border-gray-200">
                <table class="min-w-full">
                    <thead>
                        <tr>
                            <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Img Products</th>
                        ...
                        <tr>
                            <?php
                              while($row = $result->fetch_assoc()) {
                                 $id_pro = $row['id_products'];
                            
                                 // Get Img products
                                 $sql_img = "SELECT name , img , price FROM products where id = '$id_pro' ";
                                 $result_img= $conn->query($sql_img);
                                 $sql_img = $result_img->fetch_assoc();
                            ...
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <img class="h-10 w-10 rounded-full" src="../../Upload/product/<?=$sql_img['img']?>" alt="" />
                            ...
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                 <div class="ml-4">
                                        <div class="text-sm leading-5 font-medium text-gray-900"> <?=$sql_img['name']?> </div>
                                        <div class="text-sm leading-5 text-gray-500"> <?=$sql_img['price']?>$ </div>
                                    </div>
                            </td>

                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                            <div class="text-sm leading-5 text-gray-900"><?=$row['date']?></div>
                            </td>
                        </tr>
                   <?php } ?>
                    </tbody>
